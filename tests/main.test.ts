...
describe('View Options', () => {
	test('getViewOptions returns correct structure', () => {
		...
		const swimlaneToggleOption = options[3] as { displayName: string; type: string; key: string; default: boolean };
		const columnColorsOption = options[4] as { displayName: string; type: string; key: string };
		
		assert.strictEqual(options.length, 5, 'Should return five options');
		...
		assert.strictEqual(milestonesOption.displayName, 'Milestones', 'Third option should be milestone property selection');
		assert.strictEqual(milestonesOption.type, 'property', 'Milestones option should use a property picker');
		assert.strictEqual(milestonesOption.key, 'milestoneProperty', 'Milestones option key should match');
		assert.strictEqual(milestonesOption.placeholder, 'Optional list property', 'Milestones placeholder should match');
		assert.strictEqual(swimlaneToggleOption.displayName, 'Show swimlanes', 'Fourth option should be the swimlane visibility toggle');
		...
		assert.strictEqual(columnColorsOption.displayName, 'Column colors', 'Fifth option should be the column colors mapping');
